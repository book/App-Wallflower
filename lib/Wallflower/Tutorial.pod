...
create files under the F<thunk/> directory, or vice-versa. Links provided
by your application should also be to F</thunk/>. If possible,
the application should reply with status C<301 Moved Permanently> to
requests for F<thunk>, since this is what a well-behaved web servers
serving static pages will issue such a redirect.

An application that treats F</thunk> ("file") and F</thunk/> ("directory")
differently will B<not work> with L<wallflower>.
